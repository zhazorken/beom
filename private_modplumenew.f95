...
subroutine update_h()
  ...
  real( rw ) :: i_dl, hfor, rs_3, vecl( nlay ), rhsi, hfor1, hfor2, hfor3, &
          rhop1, rhop2, Wnud, Lnud, Q12, Q23, R12, R23, w12, w23, alph, Q0, gp0, B0,gp12,gp23
  real( r8 ) :: hold
...
!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(ipnt,c__1,c__3,hold,hfor,rs_3,rhsi)
 ...
 ! add water mass transformation to layer 1 and 2
      hfor1= 000._rw  ! fixed nudging top (not IC)
      hfor2= 800._rw  ! fixed nudging bot (not IC)
      hfor3= 0._rw
      if (plum > .5 .and. subc(ipnt,1)>floor(lm-Lnud/dl)) then
              
          Lnud=5000._rw
          Wnud=8000._rw  ! depends on plume width, should probably be online or in shared_mod
          alph= .13
         ! pi=4*atan(1.0_rw)
         Q0=250    ! subglacial plume outflow
          gp0=.265   ! gp at plume outflow
          B0=gp0*Q0
         ! ipnt=indc(lm,floor(real(mm/2)))
          gp12=5*B0/(6*alph)*(9/10*alph*B0)**(-1/3) * pi**(-2/3) * (hlay(ipnt,2)+hlay(ipnt,3))**(-5/3)     !reduced gravity just below interface between layers 1 and 2
          gp23=5*B0/(6*alph)*(9/10*alph*B0)**(-1/3) * pi**(-2/3) * (hlay(ipnt,3))**(-5/3)
          w12= 5/(6*alph) * (9/10*alph*B0)**(1/3)*pi**(-1/3) * (hlay(ipnt,  2)+hlay(ipnt, 3))**(-1/3)
          w23= 5/(6*alph) * (9/10*alph*B0)**(1/3)*pi**(-1/3) * (hlay(ipnt,  3))**(-1/3)
          R12=6/5*alph*(hlay(ipnt,3)+hlay(ipnt,2))
          R23=6/5*alph*hlay(ipnt,3)
          Q12=pi/2*w12*R12**2
          Q23=pi/2*w23*R23**2
          rhop1=-gp12*rhon(2)/grav + rhon(2)   !rho of plume  at interface between layers 1 and 2
          rhop2=-gp23*rhon(2)/grav+rhon(3)     !rho of plume at interface between layers 2 and 3
      end if


      if ( subc(ipnt,1)>floor(lm-Lnud/dl) ) then
         if (ilay == 1) then
           ...
           !end if
           elseif (plum> .5 .and. subc(ipnt,1)>floor(lm-Lnud/dl)) then
                   hlay( ipnt, ilay) = hlay(ipnt, ilay) + dt* Q12/(Wnud*Lnud)* &
                           (1 - max(real( (rhop1 - rhon(1))/(rhon(2)-rhon(1))), 0._rw))
           end if
         elseif (ilay ==2) then
           ! if (hfor1 > hlay (ipnt, ilay) ) then 
           if (plum <.5) then
                   hlay( ipnt, ilay) = hlay(ipnt, ilay) + 0*real(nudg(ipnt, ix_n), r8) &
                    +  max( real( hfor2 * nudg(ipnt, ix_n ), r8 )  + real( - nudg(ipnt, ix_n ), r8) * hlay(ipnt, ilay) , 0._rw )   
           elseif (plum> .5 .and. subc(ipnt,1)>floor(lm-Lnud/dl)) then
           hlay(ipnt, ilay) = hlay(ipnt, ilay) + dt*(Q23-Q12)/(Wnud*Lnud) -&
                   dt*Q23/(Wnud*Lnud)*(max(real((rhop2-rhon(2))/(rhon(3)-rhon(2))),0._rw))+ &
                   dt*Q12/(Wnud*Lnud) * (max(real((rhop1-rhon(1))/(rhon(2)-rhon(1))),0._rw))
           end if
         elseif (ilay ==3) then   
           ! if (hfor2 < hlay (ipnt, ilay) ) then
             if (plum < .5 ) then    
                hlay( ipnt, ilay) = hlay(ipnt, ilay) - 0*real(nudg(ipnt, ix_n), r8) & 
                 +  min( real( hfor3 * nudg(ipnt, ix_n ), r8 )  + real( - nudg(ipnt, ix_n ), r8) *&
                 hlay(ipnt, ilay) , 0._rw )  
           ! end if
            elseif (plum> .5 .and. subc(ipnt,1)>floor(lm-Lnud/dl)) then
               hlay(ipnt, ilay) = hlay(ipnt, ilay) - dt*Q23/(Wnud*Lnud)*&
                       (1-max(real((rhop2-rhon(2))/(rhon(3)-rhon(2))),0._rw))
             end if
        end if
 
!      elseif (hlay( ipnt, 3) < 20*hsal .and. subc(ipnt,1)>lm/2) then

!          if (ilay == 1) then
           ! if (hfor1 > hlay (ipnt, ilay) ) then 
!                hlay( ipnt, ilay) = hlay(ipnt, ilay) + 0*real(nudg(ipnt, ix_n), r8) &
!                    +  1*max( real( hfor2 * nudg(ipnt, ix_n ), r8 )  + real( - nudg(ipnt, ix_n ), r8) * hlay(ipnt, ilay) , 0._rw )   
           !end if
!         elseif (ilay ==2) then
           ! if (hfor1 > hlay (ipnt, ilay) ) then 
!                hlay( ipnt, ilay) = hlay(ipnt, ilay) - 0*real(nudg(ipnt, ix_n), r8) &
!                    +  1*min( real( hfor1 * nudg(ipnt, ix_n ), r8 )  + real( - nudg(ipnt, ix_n ), r8) * hlay(ipnt, ilay) , 0._rw )   
 
!          end if 
      end if
 ! all 3  layers must be nudged to west bdy init condition
